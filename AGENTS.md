...
- Keep recovery distinct from model repair attempts. A provider-free recovery qualification must bind its exact sealed-live corpus, complete N7 adapter digest set, checkpoint replay, duplicate-effect prevention, and zero live-provider boundary. Its canonical digest must arrive as a separate operator-owned evaluator argument; never trust a receipt and its digest from the same comparison document.
