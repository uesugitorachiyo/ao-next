...
- Recovery must carry one existing-only request-bound journal from authority comparison through effects, verification, and terminal publication. Never recreate its root, identity, or event directory; reject Windows reparse points and Unix symlinks at the root, identity, event directory, and every retained event file.
