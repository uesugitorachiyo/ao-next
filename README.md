...
The provider-gated live commands accept one [strict sealed-trial input](docs/live-evaluation-harness.md). Before any provider spawn, each variant creates the same deterministic ordinary Git repository and seed commit from the sealed product snapshot, then verifies its canonical root, common directory, branch, `HEAD`, and clean status. Immediately after the one provider process returns, AO Next retains exact bounded stdout and stderr in the input's empty operator-owned capture directory and publishes one immutable digest-bound index before normalization, AO2 preview or apply, verification, or public evidence assembly. The run record binds the Git identity, capture index, and bounded AO2 control diagnostics. Tests exercise the real local AO2 control path with a digest-bound fake provider; they never set the live-provider gate.
