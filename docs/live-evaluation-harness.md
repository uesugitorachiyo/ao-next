...
`qualify-recovery --corpus <corpus.json> --evidence-root <empty-directory>` runs the same deterministic checkpoint-replay and duplicate-effect probes without a provider or fake-provider row. Its strict receipt binds the exact corpus digest, complete N7 adapter digest set, both probe digests, successful recovery semantics, and zero live-provider processes. A comparison may carry this receipt instead of conflating recovery with model repair attempts, but `evaluate` and `evaluate-live` require its canonical digest through the separate operator-owned `--recovery-qualification-digest` argument. A receipt cannot authenticate its own digest, and a synthetic corpus cannot satisfy the recovery gate.
