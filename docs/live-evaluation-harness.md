...
`qualify-recovery --corpus <corpus.json> --evidence-root <empty-directory>` runs the same deterministic checkpoint-replay and duplicate-effect probes without a provider or fake-provider row. Its strict receipt binds the exact corpus digest, complete N7 adapter digest set, both probe digests, successful recovery semantics, and zero live-provider processes. A comparison may use this receipt instead of conflating recovery with model repair attempts; its canonical digest must match or the recovery gate remains closed.
