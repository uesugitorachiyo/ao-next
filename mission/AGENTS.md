...
- Import `ao.next.execution-journal-prefix.v1` only from a clean absolute regular-file locator outside the Mission state root. Retain exact bytes and the accepted locator, reject same-run digest conflicts before retention, and expose the result as the separate read-only `ao_next_journal_projection`. Never scan Engine state or copy Engine recovery and lifecycle execution into Mission.
...
- Decode AO Next journal prefixes with the duplicate-aware exact JSON decoder, explicit nested field whitelists, nullable token unions, typed unknown-field denial, and EOF checks. Keep projection pure and leave Mission status, route, phase, blockers, next action, workgraph, and the AO Next candidate projection unchanged.
