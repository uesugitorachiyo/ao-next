...
- Import `ao.next.execution-journal-prefix.v1` only from a clean absolute regular-file locator outside the Mission state root. Hold the cross-process Mission lock across same-run conflict detection and retention. Use the record-only transaction so `updated_at_utc`, checkpoint bytes, and event-decision bytes stay unchanged. Expose the result as the separate read-only `ao_next_journal_projection`. Never scan Engine state or copy Engine recovery and lifecycle execution into Mission.
...
- Decode AO Next journal prefixes with the duplicate-aware exact JSON decoder, explicit nested typed structures, producer enum and numeric bounds, nullable token unions, typed unknown-field denial, and EOF checks. Bind the terminal verifier digest to the preceding verifier event. Keep projection pure and leave Mission status, route, phase, blockers, next action, workgraph, and the AO Next candidate projection unchanged.
...
- AO Next journal-prefix changes: `go test ./internal/mission -run 'TestAONextJournal|TestStrictJSONTypeMatchesNullableUnion|TestValidateContractAcceptsRepositoryDiscriminators' -count=1` and `cargo test -p ao-next-core --test mission_exchange` from the repository root.
