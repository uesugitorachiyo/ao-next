...
Mission requires a clean absolute locator outside its state root. It walks every path component without following Unix symlinks or Windows reparse points, accepts only a bounded regular file, and retains the accepted locator unchanged in the artifact reference. Mission stores the exact input bytes under `artifacts/sha256/<digest>` after strict JSON, lifecycle, terminal, and digest validation.
...
Exact-byte reimport is idempotent and keeps the first accepted locator. A changed digest for the same run fails before artifact retention, so the rejection leaves no orphan object. Import preserves durable Mission status, route, phase, blockers, next action, workgraph, checkpoints, and the separate AO Next candidate projection.
